...
docker run -d -p "${BACKEND_HOST_PORT}:8080" \
  ...
  -e SPRING_PROFILES_ACTIVE="${SPRING_PROFILES_ACTIVE:-json-logs}" \
  -e ZIPKIN_ENDPOINT="${ZIPKIN_ENDPOINT:-http://tempo:9411/api/v2/spans}" \
  -e API_LOGGING_ENABLED="${API_LOGGING_ENABLED:-true}" \
